...
<script type="text/javascript" src="../../js/locales/jquery.uix.multiselect_ru2.js"></script>
...
$(function() {
    $('#user').submit(function() {
        var p1 = $("#user").find("#password").val();
        var p2 = $("#user").find("#confirmPassword").val();
...
<body>
	<div class=main>
        <div class=header>
            <span class=header-label>ПОЛЬЗОВАТЕЛЬ</span>
    		...
    		<form:form method="post" action="${command}" modelAttribute="user">
    			<table>
                    ...
                    <tr>
                        <td>Система</td>
                        <td>
                        <form:select class="input1" path="extSystem">
                                <form:option value="local">Локальные пользователи</form:option>
                                <c:forEach items="${areas}" var="area">
                                    <form1:option value="${area.id}">${area.name}</form1:option>
                                </c:forEach>
                            </form:select>
                        </td>
                        <td></td>
    				...
    				<tr>
    					...
    					<td>
                        <c:choose>
                            <c:when test="${(user.id > 10) or (empty user.id)}"><form:input class="input1" path="login" /></c:when>
                            <c:otherwise><label>${user.login}</label><form:hidden path="login" /></c:otherwise>
                        </c:choose>
                        </td>
                        <td></td>
    				</tr>
    				...
    				<tr>
    					<td>Фамилия</td>
    					<td><form:input class="input1" path="lastName" /></td>
                        <td></td>
    				</tr>
    				<tr>
    					<td>Эл.почта</td>
    					<td><form:input class="input1" path="email" /></td>
                        <td></td>
    				</tr>
    				<tr>
    					<td>Телефон</td>
    					<td><form:input class="input1" path="phone" /></td>
                        <td></td>
    				</tr>
    				<c:if test="${changePasswd}">
    				<tr>
    					<td>Пароль</td>
    					<td><form:password class="input1" path="password" /><br/>
    						<form:password class="input1" path="confirmPassword" />
    					</td>
                        <td></td>
    				</tr>
                    </c:if>
                ...
                <div class=tools-role>
                    <b>Роли:</b>
                    <form:select path="roleIds" multiple="true" class="multiselect">
                        <form:options style="margin: 12px;" items="${roleList}" />
                    </form:select>
                    <br/>
                ...
                <div class=tools-btn>
                    ...
                    <c:if test="${0 < user.id}">
                    <button onclick='window.location.href="passwd/${user.id}"' type="button">
