...
L.print.Provider = L.Class.extend({
	...
	print: function (options) {
		...
		var bounds = options.bounds;// || this._map.getBounds();
		var sw = this._projectCoords(L.print.Provider.SRS, bounds.getSouthWest());
		var ne = this._projectCoords(L.print.Provider.SRS, bounds.getNorthEast());
		...
		layers: {
			...
			tilelayer: function (layer) {
				var enc = this._encoders.layers.httprequest.call(this, layer),
				    ...
				    resolutions = [],
				    zoom;
				baseUrl = baseUrl.replace('/enk/', '/print/');
...
L.Control.Print = L.Control.extend({
	...
	_showPrintArea: function(){
		this._areaSelect = L.areaSelect({width:400, height:600, keepAspectRatio:true});
        this._areaSelect.addTo(this._map);
        this._provider.fire('printarea', {
			show: true
		});
	},
	
	_hidePrintArea: function(){
		if(this._areaSelect != null)this._areaSelect.remove();
		this._provider.fire('printarea', {
			show: false
		});
	},
		...
		for (buttonId in this._actionButtons) {
			if (this._actionButtons.hasOwnProperty(buttonId) && buttonId === id) {
				...
				this._provider.print({
					layout: button.name,
					bounds: this._areaSelect.getBounds()
				});
				break;
			}
		}
